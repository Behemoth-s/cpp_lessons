 ...
 * 格斗场实行会员制，但是新来的会员不需要交入会费，而只要同一名老会员打一场表演赛，
 * 证明自己的实力。
 *
 * 我们假设格斗的实力可以用一个正整数表示，成为实力值。另外，每个人都有一个唯一的id，
 * 也是一个正整数。为了使得比赛更好看，每一个新队员都会选择与他实力最为接近的人比赛，
...
int main()
{
    freopen("week9/part5_example.txt", "r", stdin);
    int N;
    int id, power;
    auto mems = map<int, int>();
    auto que = queue<int>();
    map<int, int>::iterator ilow;
    mems[1000000000] = 1;
    string line;
    istringstream iss;
    getline(cin, line);
